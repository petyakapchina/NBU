...
public class FilterFramework extends Thread
{
	...
	protected PipedInputStream newInputReadPort = new PipedInputStream();
	protected PipedOutputStream newOutputWritePort = new PipedOutputStream();

	protected PipedInputStream existingInputReadPort = new PipedInputStream();
	protected PipedOutputStream existingOutputWritePort = new PipedOutputStream();
	...
	****************************************************************************/
	...
	public void Connect(FilterFramework Filter)
	{
		try
		{
			...
			newInputReadPort.connect( Filter.newOutputWritePort);
			existingInputReadPort.connect(Filter.existingOutputWritePort);
			InputFilter = Filter;
	...
	*
	****************************************************************************/
	protected byte readNewFilterInputPort() throws EndOfStreamException
	{
		return readFilterInputPort(newInputReadPort);
	}

	protected byte readExistingFilterInputPort() throws EndOfStreamException
	{
		return readFilterInputPort(existingInputReadPort);
	}

	private byte readFilterInputPort(PipedInputStream inputReadPort) throws EndOfStreamException
	{
		byte datum = 0;
	...
	*
	****************************************************************************/
	protected  void WriteOriginalFilterOutputPort(byte datum){
		WriteOriginalFilterOutputPort(datum, existingOutputWritePort);
	}
	...
	protected  void WriteNewFilterOutputPort(byte datum){
		WriteOriginalFilterOutputPort(datum, newOutputWritePort);
	}

	private void WriteOriginalFilterOutputPort(byte[] datum, PipedOutputStream outputWritePort)
	{
		try
		{
	...
	****************************************************************************/
	...
	public void ClosePorts()
	{
		try
		{
			newInputReadPort.close();
			newOutputWritePort.close();
			existingInputReadPort.close();
			existingOutputWritePort.close();
